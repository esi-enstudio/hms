<x-admin-app-layout title="Retailer">
    <x-slot name="header">
                        ...
                        <table class="text-center whitespace-nowrap w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                            ...
                            @forelse($retailers as $sl => $retailer)
                                <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                                    ...
                                    <td class="px-6 py-4">
                                        <p>{{ $retailer->ddHouse->name }}</p>
                                        <p>{{ $retailer->ddHouse->code }}</p>
                                    </td>
                                    <td class="px-6 py-4">
                                        <p>{{ $retailer->name }}</p>
                                        <p>{{ $retailer->code }}</p>
                                    ...
                                    <td class="px-6 py-4">
                                        <p>Itop: {{ $retailer->itop_number }}</p>
                                        <p>RS0: {{ $retailer->rso->rso_number }}</p>
                                    </td>
                                    <td class="px-6 py-4">
                                        <p>Enabled: {{ $retailer->enabled }}</p>
                                        <p>SSO: {{ $retailer->sim_seller }}</p>
                                    </td>
